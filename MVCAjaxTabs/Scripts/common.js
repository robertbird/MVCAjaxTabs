...
(function ($) {
    ...
    */
    ...
    $.fn.addTemplateSetup(function () {
        ...
        // [Modification BMP] 08 June 2001
        // [Pupose]: convert querystring tab value to hash
        // [Reason]: 
        //  - ActionResult can contain a param in the route dictionary, but not a hash fragment
        //  - This converts the "tab" param to a hash value to load the appropriate tab        
        var tabQuery = $.query.get('tab');
        if (tabQuery) {
            window.location.hash = tabQuery;
            // note, $.query.remove('tab'); would only work if you refreshed the page
        }
    ...
    */
    $.fn.blockMessage = function (message, options) {
        ...
        this.each(function (i) {
            ...
            var finalMessage = (typeof message == 'object') ? '<ul class="' + messageClass + '"><li>' + message.join('</li><li>') + '</li></ul>' : '<p class="' + messageClass + '">' + message + '<span class="close-bt"></span></p>';
